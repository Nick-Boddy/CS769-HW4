...
def topK(score):
    ...
    flattened_score = score.view(batch_size, -1)  # [batch_size, beam_size * vocab_size]

    #  both [batch_size, beam_size]
    top_score, top_indices = flattened_score.topk(beam_size, dim=-1, largest=True, sorted=True)

    # can use vocab_size to find the indices of the original structure
    top_beamid = top_indices // vocab_size  # [batch_size, beam_size]
    top_wordid = top_indices % vocab_size   # [batch_size, beam_size]

    return top_score, top_beamid, top_wordid


def select_hiddens_by_beams(hiddens, beam_id):
    """ Rearrange the hidden state tensors according to the selected beams in the previous step
    Args:
    ...
    if len(hiddens.shape) == 3:
        ...
        expanded_beam_id = beam_id.unsqueeze(-1)  # [batch_size, beam_size, 1]
        expanded_beam_id = expanded_beam_id.expand(-1, -1, hidden_size)  # [batch_size, beam_size, hidden_size]

        new_hiddens = hiddens.gather(dim=1, index=expanded_beam_id)  # [batch_size, beam_size, hidden_size]

        return new_hiddens
    elif len(hiddens.shape) == 4:
        batch_size, beam_size, decode_max_length, hidden_size = hiddens.shape

        expanded_beam_id = beam_id.unsqueeze(-1)  # [batch_size, beam_size, 1]
        expanded_beam_id = expanded_beam_id.expand(-1, -1, decode_max_length)
        expanded_beam_id = expanded_beam_id.unsqueeze(-1)
        expanded_beam_id = expanded_beam_id.expand(-1, -1, -1, hidden_size)  # [batch_size, beam_size, decode_max_length, hidden_size]

        new_hiddens = hiddens.gather(dim=1, index=expanded_beam_id)  # [batch_size, beam_size, decode_max_length, hidden_size]

        return new_hiddens
    else:
        print('Shape is neither 3 nor 4')
...
def extract_sequences(top_score, top_wordids, top_beamids):
    ...
    Return:
        sorted_sequences: torch.Tensor, [batch_size, beam_size, decode_max_length]. word ids of final decoded sequences in the beams.
        sorted_scores: torch.FloatTensor, [batch_size, beam_size]. the sorted score of the corresponding `sequences` in the beams.
    Example: 
        See inputs and expected outputs in test_extract_sequences().
    """
    batch_size, beam_size = top_score.shape

    decode_max_length = len(top_wordids)  # the length of the longest generated sequence (the rest are padded with eos)

    sequences = torch.zeros(batch_size, beam_size, decode_max_length)

    # keep track of the beams as we go backwards
    cur_beams = torch.arange(beam_size).unsqueeze(0).expand(batch_size, -1)  # [batch_size, beam_size]

    # traverse backwards
    for i in range(decode_max_length - 1, -1, -1):
        word_ids = top_wordids[i].gather(dim=1, index=cur_beams)
        sequences[:, :, i] = word_ids

        cur_beams = top_beamids[i].gather(dim=1, index=cur_beams)

    return sequences.long(), top_score.float()
